...
function solution(players: string[], callings: string[]) {
  let ranking: any = {};
  for (let rank = 0, end = players.length; rank < end; rank++) {
    const name = players[rank];
    ranking[name] = rank;
  }
  for (const name of callings) {
    const currnetRank = ranking[name];
    ranking[name] = ranking[name] - 1;
    ranking[players[currnetRank - 1]] = ranking[players[currnetRank - 1]] + 1;
    players[currnetRank] = players[currnetRank - 1];
    players[currnetRank - 1] = name;
  }
  return players;
}
