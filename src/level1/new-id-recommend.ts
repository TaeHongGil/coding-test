...
function solution(new_id: string) {
  const level2 = /[^a-z\d-_.]/g;
  const level3 = /(\.)\1+/g;
  const level4 = /(^\.)|(\.$)/g;
  const level5 = /^$/g;
  new_id = new_id.toLowerCase().replace(level2, '').replace(level3, '.').replace(level4, '').replace(level5, 'a').substring(0, 15).replace(level4, '');
  //   new_id = new_id.length <= 2 ? new_id + new_id[new_id.length - 1].repeat(3 - new_id.length) : new_id;
